...
- **17:40 capture RESULT (checked 2026-09-24 ~18:30, [home]): SPIKE, so build it.** Phantoms (predicted stops never reached) climb from 11% at 17:40 to 45% at 17:50, stay ~18-24% through 18:01, then drop to ~5% at 18:02 when the buses are on the post-6PM route ids. Almost all from old-route ids 53 (Orange, 4 buses), 67 (Gold, 3), 68 (Green, 1). Health run also breached: 8.1% >2 min late, 8 full-lap flips, Orange Loop post-6PM median err -203s, Gold post-6PM -92s. So our engine keeps predicting the OLD route's stops for ~15-20 min after buses physically change (earlier than TransLoc's own flip). Next: parse the EVENING ROUTE CHANGE notes into `config/uts_blocks.json` and switch a bus's route by block+time (see Next step).
