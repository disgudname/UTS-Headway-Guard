...
### 2026-09-24 · [home] · W2W unassigned shifts: the full-schedule iCal feed has them; logger + all consumers COMMITTED (`2c0d6bd`) and DEPLOYED 2026-09-24 ~20:20 ET
- **Finding:** W2W's read-only API (`AssignedShiftList`) never returns unassigned shifts, but `/API/DailyPositionTotals` (documented) gives per-position/day unassigned counts+hours, and the Google-Calendar export of W2W's "Complete Schedule" (an .ics; live via the calendar's secret iCal address) lists every unassigned shift with times and notes (empty employee line). Confirmed: [08] 09-04 10:30a-6:30p unassigned (note `eb(Maggie) / eb Tyler`), 09-11 7a-11a unassigned. It is a snapshot of NOW (no history), so we log diffs.
- **Secret set by the user:** `W2W_ICAL_URL` (Fly, deployed; fetched fine from the machine: 7,103 events). Treat as a credential; never print it.
- **Built:** `w2w_schedule.py` (`parse_ics`, `diff_shifts`, `W2WScheduleLog`: snapshot + JSONL change log in the data dir, first poll = baseline, ignores LAST-MODIFIED-only restamps, ignores a feed that shrinks >50%), startup poller in `app.py` (`W2W_ICAL_POLL_S`, default 900), dispatcher-auth endpoints `GET /v1/w2w/schedule-changes` and `GET /v1/w2w/unassigned`, `tests/test_w2w_schedule.py` (7 pass; rest of suite unchanged, same 8 pre-existing failures). Ran on the real export: 6,018 tracked shifts; unassigned list sensible.
- **Deployed + live check (2026-09-24 ~20:30 ET):** `/v1/health` ok; `/data/w2w_schedule_snapshot.json` written by the first poll (6,018 shifts, baseline; change log starts empty). Ran the real `_fetch_w2w_assignments` / `_fetch_on_duty_personnel` / `_fetch_vehicle_drivers` on the machine (import app, point `w2w_schedule_log` at /data): 31 open shifts in the next 2 days; `unassigned_by_block` had 01 02 05 09 10 11 18 20 23 24 + Fan Shuttle / OnDemand Driver / [EB] / [OCEB]; 3 vehicles got an OPEN driver (bus 2 [09] and bus 30 [02] 16:00-22:30 open, bus 1 [11] OPEN 16:00-20:30 then Anna Wilcomes 20:00-22:30). Browser: /livemap and /map load with no JS errors from the changed modules; `/statussignage` `formatPersonDisplay` and the legacy `/map` `renderStatusPanel` render OPEN correctly on the deployed assets. NOT seen: the authenticated dispatcher pages (/busdispatch grid, livemap popup, van roster, eink) with real data, since they need a dispatcher login: **look at /busdispatch and a livemap bus popup next time you are logged in**. EBs are not in TransLoc (user), so an open shift is what keeps their bus listed. Rollback: previous release (`fly releases`).
- **FIX `f53f6df` (deployed ~20:35 ET): open shifts respect each caller's "today".** First version pulled open shifts for today + 2 days, so tomorrow's showed on /busdispatch as "OPEN @ 07:00" beside today's drivers. Now `open_shift_rows(now, first, last)` takes the days the caller asked W2W's API for: block-drivers / vehicle-drivers = yesterday + today (ended shifts dropped), on_duty = its one service day (rolls over at 02:30, not midnight). There are several definitions of "today" in this codebase; any new W2W consumer must pass its own. Verified live while logged in: /busdispatch grid (amber "OPEN until 22:30" on [02], "OPEN until 20:30" on [11] above Anna Wilcomes), livemap bus popup ([02] Bus 20131: "OPEN 16:00-22:30"), /vandispatch (an "OPEN - no driver assigned" OnDemand Driver card 21:30-05:30), block-drivers payload (5 open shifts, all today). `eink-block` not looked at on the live site.
- **Consumers updated for unassigned shifts (user asked, 2026-09-24; all UNCOMMITTED, not deployed):** `unassigned_by_block` added to `/v1/dispatch/block-drivers` (kept separate from `assignments_by_block` on purpose); `/v1/dispatch/vehicle-drivers` lists an `"OPEN"` `unassigned: true` driver on a bus whose block has an open shift now, labels the block with the W2W name (`[24]`, not TransLoc's `[24] PM`) and keeps the bus listed outside its TransLoc block times while an open shift runs (user: EBs usually drive open shifts); `/v1/uts/on_duty` adds `supervisors_open` / `ondemand_dispatchers_open` (`active` = uncovered now). Front ends: `html/dispatcher.html` (grid preview lines, popup, roster slot; merges open shifts into the driver lists sorted by start time), `html/eink-block.html`, `html/statussignage.html`, `scripts/livemap/*` (dispatch.js flag, vehicles.js popup `.lv-open`, panels.js status), `vandispatch2` (data.js + duty-roster.js "Open" cards), legacy `scripts/testmap.js` status panel (+ rebuilt `.min.*` via `npm run build:testmap-min`). Legacy `/map` bus popup shows the OPEN entry as the driver name with no special styling. `vehicle_drivers/uva.py` is dead code (imported, never used), left alone. Tests: `tests/test_w2w_schedule.py` (14; covers open_shift_rows, `_build_driver_assignments` flag, `_fetch_vehicle_drivers` with open shifts, `_fetch_on_duty_personnel`); JS only syntax-checked with `node --check` plus a node harness for the dispatcher/eink normalizer and the statussignage formatter. NOT verified in a browser (dispatcher pages need auth). Default poll is now 300 s. After deploy: the log starts from a baseline on the first poll, so `unassigned_by_block` is empty until that poll lands (~20 s after boot).
- **Not done:** commit + `fly deploy`. Scripts `scripts/w2w_ics.py`, `scripts/ridership_pull.py`, `scripts/block8_ridership.py` are also uncommitted. The .ics and `data-local/ridership/w2w_block8_shifts.json` contain staff names: keep out of git.

### 2026-09-24 · [home] · Evening route change: hide stops the post-6PM route skips (COMMITTED `da27438`, DEPLOYED 2026-09-24 ~18:55 ET (release after v2004), health ok + /v1/eta/uts_stop_arrivals serving; NOT yet seen on a live change window)
- **Problem (measured on the 17:40 capture):** until TransLoc flips a bus's route id (17:45/17:51/17:57, a fixed clock time, ~1-2 min after the schedule's change, wherever the bus is), we predict it at pre-6PM stops the evening route skips (Emmet St, University/Newcomb, some JPA...). 54 (bus, stop) pairs wrongly shown for a median 5-8 min; 34% of Green's stop-level "next bus" readings, 11% Gold, 2% Orange. TransLoc shows the same stale data, worse.
- **Rule** (`bus_eta.route_change_hidden_stops`, wired in `app.py` `_compute_bus_eta_arrivals`): per bus with an evening-change note, hide stops the post-6PM route does NOT serve (matched by stop NAME, IDs are renumbered per variant) that come after the change stop (the note's "after leaving HER/BAR/MP/PIN at HHMM") on the change trip; once the bus is at/just past that stop at/after the time, hide all of them. Earlier-trip buses (ETA at the change stop closer to the block's previous scheduled visit than to the note time) keep everything. No plan / no ETA / no shape = no-op. `_route_change_block_memory` bridges the 1-2 min where dispatch's block for the bus drops out.
- **Data:** `build_uts_blocks.py` now parses the "EVENING ROUTE CHANGE" boxes into `route_change` on each weekday group of `config/uts_blocks.json` (8 blocks: Gold 09/10/11/12, Green 01/02, Orange 05/07; regenerated, diff is additive). `config/evening_route_stops.json` = names of stops each post-6PM route serves (Gold 67->57, Green 68->54, Orange 53->55), built from a prod `<day>_routes.json` snapshot by `scripts/build_evening_route_stops.py`. Re-run it if the evening routes change.
- **Test:** 12 new unit tests (bus_eta, uts_blocks, build_uts_blocks); rest of the suite unchanged (8 failures in test_cache_concurrency/test_ondemand_filters and a collection error in test_vehicle_drivers are PRE-EXISTING, same on a clean tree). `scripts/eta_route_change_check.py <log>` replays the rule on an eta_watch log: on 09-24 17:40, of 1,844 wrong predictions it hides 1,712 (93%) and hides 0 stops the bus really served before its flip. The real `_compute_bus_eta_arrivals` was also run offline against that log's positions (rows per bus drop e.g. Green bus 1 24 -> 8 at 17:44, nothing changes after the flip).
- **Not covered / caveats:** only ONE evening of our own predictions exists (09-21..23 have positions only), so false-hide risk on late/early buses is reasoned, not measured. Orange [07] bus 13 still shows ~100 wrong readings for Stadium Rd stops before PIN (it seems to skip them before its noted change). Blocks 06/08 (Orange, going out of service at 18:00) are the separate out-of-service cut-off, untouched: TransLoc drops them to route 0 at ~18:01 while they still run a last in-service stretch to LIB/MP, so those riders lose the bus (open question, not addressed). Weekends have no note = no-op.
- **Orange blocks 06/08 (investigated 2026-09-24 evening, [home]; fix COMMITTED `4103de2` + DEPLOYED 2026-09-24 ~19:05 ET as release v2006; health ok, ETA feed 87 entries after warm-up):** their OUT-OF-SERVICE notes ("leave MP/CSW at 1800, in service until LIB/MP, return to lot") ARE the evening change for those blocks: from 3 evenings of bus tracks, bus 45 [06] leaves MP ~18:01 and runs the POST-1800 path (Grady 10 1/2 / 14th / Preston Pl, Rugby, Library 18:07-18:10, Monroe Hall, McCormick Dorms) then the lot; bus 14 [08] leaves Stadium ~18:01 -> Stadium Rd -> Alderman Rd @ Gooch/Dillard -> O-Hill Dining -> lot by ~18:05 (never reaches MP, so the note's "until MP" does not match reality). TransLoc sets both to route 0 at ~18:01 (fixed clock time), so they get NO ETAs from anyone for that last stretch. Before the change our engine kept predicting the old-route stops for them (17:44-18:01), which is where most of the 'never reached' predictions on route 53 came from. **Change (uncommitted):** `uts_blocks.route_change_plan` falls back to a block's out-of-service leave stop/time when it is 17:30-18:30 (only [06]/[08] qualify), so the hide-skipped-stops rule covers them. Replay on the 09-24 17:40 log: 2,683 of 2,815 wrong predictions hidden (95%), none hidden that the bus served. **Decided NOT to do (user, 2026-09-24):** no predictions for buses TransLoc marks out of service (route 0). Bus 14 [08] (fleet 18232) not going to MP is a driver deviation from the note (user is handling it), not something to model. Rollback target if needed: v2005 image (first route-change release).
- **Watch:** new Task Scheduler job `ETA-RouteChange-Weekdays` (Mon-Fri 17:30, `eta_health_check.py 60 15`, logs `data-local/eta_watch/<date>-1730.jsonl` + `.graph.json`, first run Fri 2026-09-25). Nothing reads it automatically: afterwards run `python scripts/eta_phantoms.py <log>` (still overcounts: old-id predictions that come due after the flip) and `python scripts/eta_route_change_check.py <log>` (now measures RESIDUAL wrong predictions with the rule live; expect far fewer than the 1,844 seen without it, and check nothing is hidden that the bus served). Roll back with `fly releases` / `fly deploy --image` to the v2004 image if evening ETAs look wrong.
...
- **17:40 capture RESULT (rechecked twice 2026-09-24 evening, [home]): NOT settled; `eta_phantoms.py` OVERCOUNTS but real phantoms exist.** The script counts any prediction whose (route id, stop id, bus) key never resolves, so predictions made under the old route id (53/67/68) that come due after TransLoc's flip are counted even when the bus physically passed the stop (checked vs bus tracks). Measured properly (bus never within 100 m of the stop, predictions up to 30 min out): 12.6% of 21.6k predictions, 121 distinct (route,bus,stop), mostly Orange 53 buses 45 [06] and 14 [08] predicting JPA/Maury/Madison Ave/14th St stops for ~17 min (17:44-18:01) that they never reached; Gold 67 and Green 68 ~470 obs each. Ends when the route id flips (18:01). Blocks 06/08 have NO evening-change note, so the cause is unknown (short turn? detour? cut-off?). Rider-visible impact of phantoms: real. NEXT: look at where buses 45 and 14 actually went 17:44-18:01 (tracks in the log's `veh`) before choosing a fix. Separately, the >2 min-late breaches are ordinary rush-hour lateness (Purple 73, Silver 58, Gold-pre 67), same rate in the 17:00 run with no route change.
