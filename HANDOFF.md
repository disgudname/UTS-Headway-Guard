...
### 2026-09-24 · [home] · Kiosk flapping SOLVED (root cause is in the kiosk repo, not the backend); fix drafted locally, NOT pushed
...
- **Rollout:** kiosks self-update from git branch = their channel (`dev`/`prod`), not master. Push to `dev` first, then promote to `prod` (the flapping unit is on prod). Needs user go-ahead.
