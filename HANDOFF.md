...
- **Added (2026-09-24, deployed): `[loop-lag]` watchdog** in `app.py` (`_loop_lag_watchdog`) logs `[loop-lag] event loop blocked for Xs` whenever the event loop stalls >= 0.5 s. Use it to decide server-stall vs network: `flyctl logs` (short window!) and grep `loop-lag` around a kiosk flap. Also measured 2026-09-24: machine idle (load 0.2, 1.6 GB free of 2 GB), so no sustained overload. If no stalls line up with flaps, next step is retry/tolerance in the kiosk's check-in code (separate chilipie-kiosk-modern repo).
