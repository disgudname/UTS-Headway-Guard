...
- **17:40 capture RESULT (rechecked 2026-09-24 ~19:00, [home]) - CORRECTS an earlier entry that said 'spike, build it': DON'T BUILD.** `eta_phantoms.py` counts a prediction as a phantom when its (route id, stop id, bus) key never resolves, so every prediction made under the OLD route id (53/67/68) that comes due after TransLoc's flip (17:51 / 17:57 / 18:00) is counted, even though the bus DID physically pass that stop (checked against bus tracks: only ~65 true never-reached predictions in 40 min, ~2/min, flat, no spike; 45 of 69 'phantoms' on route 53 were stops the bus really passed). After the flip, route ids 54/55/57 score ~0% late. The health-check breaches are ordinary rush-hour lateness (>2 min late) on Purple 73 (bus 34/36/24/19), Silver 58 (bus 38/26) and Gold-pre 67 (bus 33/42, 17:00 run), mostly 'historical'-source predictions 10+ min out. Same rate in the 17:00 run (8.5%) that had no route change. Route-change notes are not the fix. Open question: why are historical/long-range predictions late at rush hour (buses slower than the history average)?
