...
# Route "54": weekdays layover at MP in the daytime and CHP in the evening; weekends
# only at CHP. Route "59" has no schedule data at all. All three codes are mapped.
ACTIVE_BLOCKS = {
    "[01]": {
        "route_ids": ["54"],
        "weekday_groups": [
            {"weekdays": [0, 1, 2, 3, 4], "stops": [[9 * 3600, "MP"], [10 * 3600, "MP"], [19 * 3600, "CHP"], [20 * 3600, "CHP"]]},
            {"weekdays": [5, 6], "stops": [[9 * 3600, "CHP"], [10 * 3600, "CHP"]]},
        ],
    },
}
ACTIVE_TIMESTOPS = {"MP": {"54": "657"}, "CHP": {"54": "653", "59": "743"}}


def test_timestop_active_follows_day_group_and_time_of_day(monkeypatch):
    _patch_data(monkeypatch, ACTIVE_BLOCKS, ACTIVE_TIMESTOPS)
    mon_day = _epoch(2026, 9, 14, 9, 10)   # Monday morning
    mon_eve = _epoch(2026, 9, 14, 19, 10)  # Monday evening
    sun_day = _epoch(2026, 9, 20, 9, 10)   # Sunday morning
    assert uts_blocks.is_timestop_active("54", "657", mon_day) is True     # MP daytime weekday
    assert uts_blocks.is_timestop_active("54", "657", mon_eve) is False    # MP not a timestop in the evening
    assert uts_blocks.is_timestop_active("54", "653", mon_eve) is True     # CHP evening weekday
    assert uts_blocks.is_timestop_active("54", "653", mon_day) is False
    assert uts_blocks.is_timestop_active("54", "657", sun_day) is False    # weekend: MP not scheduled
    assert uts_blocks.is_timestop_active("54", "653", sun_day) is True     # weekend: CHP is


def test_timestop_active_unmapped_stop_is_never_a_timestop(monkeypatch):
    _patch_data(monkeypatch, ACTIVE_BLOCKS, ACTIVE_TIMESTOPS)
    assert uts_blocks.is_timestop_active("54", "999", _epoch(2026, 9, 14, 9, 10)) is False


def test_timestop_active_route_without_schedule_keeps_mapping(monkeypatch):
    _patch_data(monkeypatch, ACTIVE_BLOCKS, ACTIVE_TIMESTOPS)
    # Route 59 is mapped but no block serves it -> nothing to contradict the mapping.
    assert uts_blocks.is_timestop_active("59", "743", _epoch(2026, 9, 14, 9, 10)) is True
