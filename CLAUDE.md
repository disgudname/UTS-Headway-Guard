...
**Route/ETA formatting in RSS/CAP:** `_strip_route_suffix()` drops a trailing "Loop"/"Line" word from route names, then `_abbreviate_route()` maps the result to UTS's own official signage short names where one exists (`Green`→`GRN`, `Night Pilot`→`NP`, `Orange`→`OR`, `Gold`→`GOLD`, `Purple`→`PRP`, `Silver`→`SLV`, via `_ROUTE_ABBREVIATIONS`) — routes without an official short name (fan shuttles, Charter, Training, test routes) pass through unabbreviated. Red and Blue are decommissioned (replaced by Purple) so they're intentionally absent from the map. `_arrival_label()` renders times as `"3m"` not `"3 min"` (and `"0m"` for an arriving bus, not `"Due"`), and route:times are joined with no space around the colon/commas (e.g. `"PRP:2m,5m"`) so a single route is one atomic, space-free token. Multiple routes (and any active alert text) are joined with a single space, not punctuation. Abbreviating route names bought back enough headroom that both ETAs are kept regardless of digit count for every route *except* Gold: `_trim_gold_double_digit_eta()` drops Gold's second ETA only when *both* ETAs would be double-digit (labels are soonest-first, so a double-digit first ETA implies the second is too, e.g. `"GOLD:12m,45m"` → `"GOLD:12m"`) — a single-digit first paired with a double-digit second (e.g. `"GOLD:8m,20m"`) is short enough to keep both. `GOLD` is the longest route abbreviation (4 characters), so only its worst case (two double-digit ETAs back to back) risks the mid-token wrap that longer full route names (e.g. the old `"Orange:8m,20m"`) triggered on the Novanex OnAlert sign's pixel-width word-wrap. The other, shorter abbreviations (`GRN`, `OR`, `NP`, `PRP`, `SLV`) have enough headroom to keep both ETAs unconditionally. This does **not** guarantee one route per line — two short routes can still land on the same line together if both fit, since the wrapper packs greedily and has no concept of "these are different routes." There is no way to force a hard line break from feed content (embedded newlines get collapsed to a space by the sign, not treated as breaks).
