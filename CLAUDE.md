...
**Route/ETA formatting in RSS/CAP:** route names are used as-is from TransLoc's `RouteDescription` (e.g. `"Purple Line"`, `"Night Pilot"`) — no suffix-stripping or abbreviation. `_arrival_label()` renders times as `"3min"` not `"3 min"` (and `"0min"` for an arriving bus, not `"Due"`). A route's ETAs are comma-joined with no space (`"0min,13min"`), then joined to the route name with a single space and no colon (e.g. `"Purple Line 0min,13min"`); multiple routes (and any active alert text) are likewise joined with a single space. There is no length-based trimming of route names or ETAs — earlier iterations tried keeping each route as one atomic space-free token (and later, capping double-digit ETAs) to dodge the Novanex OnAlert sign's pixel-width word-wrap, but once the sign's actual line/page layout was understood directly, the tradeoff flipped: plain spaces are now used deliberately as the wrap points rather than being avoided, so ordinary space-separated full names and times are used throughout. There is no way to force a hard line break from feed content (embedded newlines get collapsed to a space by the sign, not treated as breaks).
