...
**Route/ETA formatting in RSS/CAP:** `_strip_route_suffix()` drops a trailing "Loop"/"Line" word from route names, `_arrival_label()` renders times as `"3m"` not `"3 min"`, and route:times are joined with no space around the colon/commas (e.g. `"Purple:2m,5m"`). Multiple routes (and any active alert text) are joined with a single space, not punctuation. This is deliberately tight, not just cosmetic — it's sized so a Novanex sign renders one line per route and one route per line, with the sign's own text wrapping breaking cleanly at the space between routes, instead of a route's arrivals wrapping past the sign's character width.
